...
| 项目 | 固定版本 | 许可证 | 用途 |
|---|---|---|---|
| UTM | commit `04c4f2be115a0bd9e632a53af1db18a38e9d74d6` | Apache-2.0 / GPL-2.0-or-later（按组件） | QEMUKit、显示、输入和 QEMU iOS 构建基础 |
...
`scripts/fetch-qemu-deps.sh` 只拉取固定 UTM commit。没有可验证产物时，构建脚本会失败并明确报告，而不是生成来源不明的二进制。
