...
当前仓库可以构建基础无签名 IPA。Android VM 的产品接口、状态机、Runtime 校验和 JIT 探测已经实现，但 UTM/QEMU 可执行核心、SPICE 显示、内置 ADB 与 Android 镜像尚未链接，因此普通 APK 会显示明确的“QEMU Core 未嵌入”，不会假装启动成功。Ren'Py Runtime 同样需要后续按许可证和体积独立构建。
