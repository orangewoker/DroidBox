...
final class ImportCoordinator {
    ...
    private func importFile(_ source: URL) async throws -> GameRecord {
        ...
        set(.icon,0.40);let abi=ABIAnalyzer.analyze(paths:paths);set(.engine,0.55);let detected=EngineDetector.detect(paths:paths);set(.compatibility,0.70)
        ...
        set(.runtime,0.9);try Task.checkCancellation();let title=(manifest.label?.hasPrefix("@") == false ? manifest.label:nil) ?? source.deletingPathExtension().lastPathComponent
